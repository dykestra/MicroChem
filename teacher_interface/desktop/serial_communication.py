...
try:
    while True:
        ...
        if comm_type == IDALIAS: ##NEED TO IMPLEMENT CHANGES TO mb and mb_hub codes
            SERIAL = id_in
        ...
        #  find "id_in" in the master table, and pick the element given to it, if any 
            ...
            #print("STUB -- check reaction table for reactions")
            message_s = message.split(b"#")
            id_a = message_s[0]
            id_b = message_s[1]
            x, = np.where(master_table["aliasID"] == id_a)
            ...
            if reaction_tablae[j,i] != b"":
                ...
                master_table["valence"][y] = b'+0'
                s.write(b'1' + b'\n') ##OK message
                ## THE HUB CAN ACCEPT THE OK, AND THEN REQUEST THE SYMBOLS AS BEFORE!
